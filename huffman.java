...
//Clase Nodo de arbol de huffman
 ...
 //Clase Nodo de una lista (lista de �rboles)
 class NodoLista { 
 ...
 void CargarTablaDeArchivo(String NombreArchivo) {
	...
	//Crear un nodo raiz de arbol que contiene DATOS (flag en true)
	 ...
	 MostrarListaDeArboles();
	  ...
	  // Proceso para reducir la lista a un solo nodo con un solo �rbol
	 ...
	 //en reemplazo de los 2 eliminados
	  ...
	  int i=0;
	  void GenerarCodigosDeHuffman_recursivo(NodoArbol AUXNODORAIZ,String clave) {
		  if(AUXNODORAIZ!=null) {
			  if(AUXNODORAIZ.clave!='*') {
				TablaHuffmanCodigos[indice]=clave;
				i++;
			  }else {
				GenerarCodigosDeHuffman_recursivo(AUXNODORAIZ.izquierda,clave=clave + "0");
				  GenerarCodigosDeHuffman_recursivo(AUXNODORAIZ.derecha,clave=clave+"1"); 
	 ...
	 IOException {
	  ...
	  void GenerarArchivoComprimido(String NombreArchivoO,String 
			  NombreArchivoD) {
			  ...
			  public static void main(String[] args) {
				  ...
				  AHuffman.Comprimir("G:\\mi unidad\\UNIVERSIDAD SIGLO 21\\algoritmo 2\\TP4\\prueba.txt"); 
				  }
}
